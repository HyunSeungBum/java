...
if [ "$#" -ne 2 ]; then
	echo "Usage: $0 ManagedServerName [start|stop]"
	exit 1
fi
...
WEBLOGIC_NAME=$MANAGED_SERVER
MANAGED_SERVER_ENV="$DOMAIN_HOME/init.d/${WEBLOGIC_NAME}.env"
DESC="Weblogic10.3.6 Application Server for $WEBLOGIC_NAME"

CLEAR_WORK_TMP="Y"

# Fetch Managed Server env
if [ -r "$MANAGED_SERVER_ENV" ]; then
	. "$MANAGED_SERVER_ENV"
fi
...
function cleanCacheTmp()
{
	...
	for i in `ls $DOMAIN_HOME/servers/$SERVER_NAME/tmp/_WL_user`
	do
		rm -rf $DOMAIN_HOME/servers/$SERVER_NAME/tmp/_WL_user/${i}
	done

	#rm -rf $DOMAIN_HOME/servers/$SERVER_NAME/tmp/_WL_user/*
}
...
START_SCRIPT="$DOMAIN_HOME/bin/startManagedWebLogic.sh"
...
start)
	...
	if [ $status_start -eq 3 ]; then
		...
		su -l $WL_USER -c "source $MANAGED_SERVER_ENV; $START_SCRIPT $WEBLOGIC_NAME > ${WLS_REDIRECT_LOG} 2>&1 &"
...
restart)
	...
	if [ $status_restart -eq 0 ]; then
		$0 $WEBLOGIC_NAME stop
	fi
	
	$0 $WEBLOGIC_NAME start
;;
